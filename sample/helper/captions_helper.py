...
from helper.helper import AwsHelper, S3Helper
...
class Captions:
    ...
    def TranslateCaptions(self, translationContext, terminology_names=[]):
        ...
        try:
            ...
            response = translate_client.start_text_translation_job(
                ...
                OutputDataConfig={"S3Uri": "s3://{}/{}".format(bucket, outputPath)},
    ...
    def captionsToSRT(self, captions):
        ...
        for caption in captions:
            srt += str(index) + "\n"
            srt += (
                self.formatTimeSRT(float(caption["start"]))
                + " --> "
                + self.formatTimeSRT(float(caption["end"]))
                + "\n"
            )
            srt += caption["caption"] + "\n\n"
            index += 1
    ...
    def captionsToVTT(self, captions):
        ...
        for caption in captions:
            vtt += (
                self.formatTimeVTT(float(caption["start"]))
                + " --> "
                + self.formatTimeVTT(float(caption["end"]))
                + "\n"
            )
            vtt += caption["caption"] + "\n\n"
    ...
    def DelimitedToWebCaptions(
        self, sourceWebCaptions, delimitedCaptions, delimiter, maxCaptionLineLength
    ):
    ...
    def vttToCaptions(self, vttObject):
        ...
        try:
            self.logger.debug(
                "Getting data from s3://" + vttObject["Bucket"] + "/" + vttObject["Key"]
            )
