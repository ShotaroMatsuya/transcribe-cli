...
def amazon_transcribe(audio_file_name="test.mp4"):
    ...
    job_uri = "s3://smat-transcription-input/Usage+of+S3+Buckets.mp4"
    ...
    return [transcript_output, subtitle_output]
...
def download_object(s3_client, bucket_name, object_name):
    ...
    try:
        s3_client.download_file(
            Bucket=bucket_name, Key=object_name, Filename="./download.json"
        )
...
def main():
    # res = amazon_transcribe()
    # s3_transcript_summary = res[0]
    # s3_subtitle_vtt = res[1]
    ...
    delete_object(s3_client, "smat-transcription-input", "Usage+of+S3+Buckets.mp4")
    get_object_list(s3_client, "smat-transcription-input")
